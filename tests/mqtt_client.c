...
int mqtt_build_publish_packet(uint8_t *buffer, size_t buffer_size, const char *topic, const char *message, uint16_t packet_id) {
    size_t topic_len = strlen(topic);
    ...
    buffer[0] = (MQTT_PUBLISH << 4);
    memcpy(buffer + 1, rem_len_buf, rem_len_size);

    buffer[1 + rem_len_size] = (topic_len >> 8) & 0xFF;
    buffer[2 + rem_len_size] = topic_len & 0xFF;
    memcpy(buffer + 3 + rem_len_size, topic, topic_len);
    memcpy(buffer + 3 + rem_len_size + topic_len, message, message_len);

    return 1 + rem_len_size + remaining_len;
}

// Publish a message to a topic
int mqtt_publish(mqtt_client_t *client, const char *topic, const char *message, uint16_t packet_id) {
    uint8_t buffer[256];
    int packet_len = mqtt_build_publish_packet(buffer, sizeof(buffer), topic, message, packet_id);
    if (packet_len < 0) {
        fprintf(stderr, "Failed to build PUBLISH packet\n");
        return -1;
    }

    if (send(client->sockfd, (char *)buffer, packet_len, 0) != packet_len) {
        fprintf(stderr, "Failed to send PUBLISH packet: %d\n", WSAGetLastError());
        return -1;
    }
...
int mqtt_ping(mqtt_client_t *client) {
    ...
    while (1) {
        ...
        if (packet_type == MQTT_PINGRESP && remaining_len == 0) {
            break; // Got PINGRESP
        }

        // Handle other packets (e.g., PUBLISH)
...
int main() {
    mqtt_client_t *client = mqtt_client_init("broker.hivemq.com", 1883, "test-client");
    if (!client) {
    ...
    if (mqtt_publish(client, "commands", "Test message from mqtt_client", 2) < 0) {
        mqtt_client_free(client);
        WSACleanup();
        return 1;
    }
    printf("Published test message to 'commands'\n");
